...
;;;###autoload
(define-minor-mode org-babel-entangle-mode
  "Global minor mode for org-babel-entangle post-tangle fixups.
When enabled, automatically applies fixups after `org-babel-tangle'
for org files generated by `org-babel-entangle-directory'."
  :global t
  :group 'org-babel-entangle
  (if org-babel-entangle-mode
      (advice-add 'org-babel-tangle :after #'org-babel-entangle--after-tangle)
    (advice-remove 'org-babel-tangle #'org-babel-entangle--after-tangle)))
...
(defvar org-babel-entangle-output-file nil
  "Output file path, bound during `org-babel-entangle-after-directory-hook'.")

(defvar org-babel-entangle-after-directory-hook nil
  "Normal hook run after `org-babel-entangle-directory' generates an org file.
`org-babel-entangle-output-file' is bound to the output path.")
...
When called interactively, prompts for DIRECTORY and OUTPUT-FILE."
  ...
  (let* ((dir (file-name-as-directory (expand-file-name directory)))
         ...
         (all-fixups nil))
    (unless entries
      (user-error "No eligible source files found in %s" dir))
    ...
    (let ((encoded-entries nil))
      ...
      (let ((org-content (org-babel-entangle--write title encoded-entries all-fixups)))
        ...
        (let ((org-babel-entangle-output-file output))
          (run-hooks 'org-babel-entangle-after-directory-hook))
        output))))
...
Usage: emacs --batch -l org-babel-entangle.el \\
  ...
  (let* ((dir (or (car command-line-args-left)
                  (user-error "Usage: org-babel-entangle-batch DIR [OUTPUT]")))
